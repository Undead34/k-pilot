...
from k_pilot.core.domain.media.models import PlaybackStatus, RepeatMode, ShuffleMode
...
async def control_media(
    ...
    """
    logger = get_logger("k-pilot.media_tools")
    ...
    if action in ("play", "pause", "toggle"):
        ...
        return f"{icons[action]} {result.message}" if result.success else f"✗ {result.message}"

    elif action == "next":
        result = await port.next_track(player_name=player_name)
    elif action == "previous":
        result = await port.previous_track(force=force_previous, player_name=player_name)
        logger.info(
...
async def get_now_playing(ctx: RunContext[AppDeps]) -> str:
    ...
    return "\n".join(lines) if len(lines) > 1 else "🎵 No hay información multimedia disponible"
...
async def seek(
    ...
    return f"{'✓' if result.success else '✗'} {text}" if result.success else f"✗ {result.message}"
...
async def toggle_shuffle(ctx: RunContext[AppDeps], player_name: str | None = None) -> str:
