...
class MprisMediaAdapter(MediaControlPort):
    ...
    def __init__(self, bus: SessionMessageBus | None = None) -> None:
        ...
        logger = get_logger("k-pilot.mpris_adapter")
    ...
    async def list_players(self) -> list[MediaPlayer]:
        ...
        self._failed_buses = defaultdict(
            int,
            {name: count for name, count in self._failed_buses.items() if name in alive},
        )
    ...
    def _list_mpris_buses(self) -> list[str]:
        ...
        try:
            ...
            mpris_buses = [
                str(name) for name in names if str(name).startswith("org.mpris.MediaPlayer2.")
            ]
    ...
    def _safe_get_prop(self, props: PropertiesProxy, prop_name: str, default: Any) -> Any:
